...
async def register_user(user: UserModel):
    user_db = await mongo.find_user(user.username)
    if user_db:
...
async def login_user(user: UserLoginModel):
    user_from_db = await mongo.find_user(user.username)
...
async def get_my_profile(x_username: Union[str, None] = Header(default=None)):
    return await mongo.find_user(x_username)


@router.get("/profile/{username}")
async def get_user_profile(username: str):
    return await mongo.find_user(username)
